...
6. The **Fixer** isn't part of a normal build — it's invoked by the verify-and-fix loop (`verify/BuildVerifier.ts`) when a build/test run fails, given the error output and the manifest.
...
9. After a batch is approved and applied, click **Verify build** to run the project's install/build/test commands and, on failure, have the Fixer propose a correction — capped at 5 iterations. A per-project **auto-approve** toggle (default off) lets this loop iterate unattended instead of pausing for manual approval at every step.
10. Approvals, projects (and their auto-approve setting), activity log entries, agent run records, and verify-loop iterations all live in SQLite (`lib/db/`) — a server restart doesn't lose any of it.
...
- `verify/BuildVerifier.ts` — the verify-and-fix loop; `verify/AutoApproveStore.ts` — per-project auto-approve toggle.
- `../lib/db/` — SQLite connection (`connection.ts`, WAL mode, schema in `schema.ts`) and repositories (`approvalsRepo`, `projectsRepo`, `activityRepo`, `agentRunsRepo`, `verifyIterationsRepo`) backing all of the above.
